...
namespace :assets do
  ...
  task :compile_js do
    ...
    outfile   = Pathname.new('js').join('vendor.js') # may want to use the digest in the future?
    FileUtils.mkdir_p outfile.dirname
    asset.write_to(outfile)

    asset     = sprockets['site']
    outfile   = Pathname.new('js').join('site.js') # may want to use the digest in the future?
    FileUtils.mkdir_p outfile.dirname
    asset.write_to(outfile)
